...
var collectionMenu = db.ref().child("Menu");
     
var dataSet = [];//array para guardar los valores de los campos inputs del form
/*datatable*/
var tableMenu = $('#tablaMenu').DataTable({
        pageLength : 5,
...
collectionMenu.on("child_added", datos => {        
    /*Al insertar datos pinta la fila añadida, en lugar de realizar todas las consultas*/
    dataSet = [datos.key, datos.child("food").val(), datos.child("description").val()];
    tableMenu.rows.add([dataSet]).draw();
});
collectionMenu.on('child_changed', datos => {           
    /*Al actualizar datos pinta la fila editada, en lugar de realizar todas las consultas*/
    dataSet = [datos.key, datos.child("food").val(), datos.child("description").val()];
    tableMenu.row(filaEditada).data(dataSet).draw();
});
collectionMenu.on("child_removed", function() {
     /*Eliminar la fila eliminada, en lugar de realizar todas las consultas*/
    tableMenu.row(filaEliminada.parents('tr')).remove().draw();                     
});
      
$('form').submit(function(e){         
    /*obtener datos del modal*/                
    e.preventDefault();
    let id = $.trim($('#Menu_id').val());        
    let food = $.trim($('#Menu_food').val());
    let description = $.trim($('#Menu_description').val());                
    /* 
        obtener id
        si existe actualiza
    ...
    */
    ...
    if (idFirebase == ''){       
        ...
        idFirebase = collectionMenu.push().key;
    };    

    /*crea un objeto que será la fila que se va a insertar o actualizar*/            
    data = {food:food, description:description};    
    console.log(data);         
    actualizacionData = {};
    actualizacionData[`/${idFirebase}`] = data;
    collectionMenu.update(actualizacionData);
