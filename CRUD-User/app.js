$(document).ready(function() {
    ...
    var collectionUsers = db.ref().child("User");
    var collectionBitacora = db.ref().child("Bitacora");

         
    var dataSet = [];//array para guardar los valores de los campos inputs del form
    /*datatable*/
    var tableUser = $('#tablaUser').DataTable({
            pageLength : 5,
    ...
    collectionUsers.on("child_added", datos => {        
        /*Al insertar datos pinta la fila añadida, en lugar de realizar todas las consultas*/
        dataSet = [datos.key, datos.child("email").val(), datos.child("password").val()];
        tableUser.rows.add([dataSet]).draw();
    });
    collectionUsers.on('child_changed', datos => {           
        /*Al actualizar datos pinta la fila editada, en lugar de realizar todas las consultas*/
        dataSet = [datos.key, datos.child("email").val(), datos.child("password").val()];
        tableUser.row(filaEditada).data(dataSet).draw();
    });
    collectionUsers.on("child_removed", function() {
         /*Eliminar la fila eliminada, en lugar de realizar todas las consultas*/
        tableUser.row(filaEliminada.parents('tr')).remove().draw();                     
    });
    
    function actualizarBitacora(){
        var bit_user = "Firebase";
        var bit_action = "Se ha añadido a la tabla user";
        var bit_fecha = new Date();

        bit_data = {bit_user:bit_user, bit_action:bit_action, bit_fecha:bit_fecha};
        
        let bit_id = collectionBitacora.push().key;

        actualizacionData = {};
        actualizacionData[`/${bit_id}`] = bit_data;
        collectionBitacora.update(actualizacionData);
    }
    ...
    $('form').submit(function(e){         
        ...
        */
        ...
        if (idFirebase == ''){       
            ...
            idFirebase = collectionUsers.push().key;
        };    
        ...
        collectionUsers.update(actualizacionData);
        actualizarBitacora();
    ...
    $("#tablaUser").on("click", ".btnEditar", function() {    

        /*Obtener los datos de la fila de la tabla*/
        filaEditada = tableUser.row($(this).parents('tr'));           
        let fila = $('#tablaUser').dataTable().fnGetData($(this).closest('tr'));               
        let id = fila[0];
    ...
    $("#tablaUser").on("click", ".btnBorrar", function() {   
